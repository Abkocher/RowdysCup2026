...
document.addEventListener('mousemove', e => {
  if (!persistentOpenBottom) {
    if (e.clientY >= window.innerHeight-20) {
      sidebarBottom.classList.add('open'); sidebarBottom.classList.remove('collapsed');
    } else if (e.clientY < window.innerHeight-180) {
      sidebarBottom.classList.remove('open'); sidebarBottom.classList.add('collapsed');
    }
  }
});
...
document.getElementById("home-score").addEventListener("click",
  e=>updateScore("home", e.offsetY, e.currentTarget)
);
document.getElementById("guest-score").addEventListener("click",
  e=>updateScore("guest", e.offsetY, e.currentTarget)
);

// Tastatursteuerung für Fire TV-Knopf: 1,4 = Heim +/-, 2,5 = Gast +/-
